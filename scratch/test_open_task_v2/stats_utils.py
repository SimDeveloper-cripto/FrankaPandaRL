...
def compare_continuous(
    ...
    a = np.asarray(a, float)
    b = np.asarray(b, float)
    # Campioni DEGENERI: se entrambi hanno varianza nulla, o se sono identici, il t di
    # Welch è indefinito (0/0) e scipy emette "catastrophic cancellation" restituendo un
    # p-value inaffidabile. Casi reali in questa suite: un'ablazione in cui TUTTI gli
    # episodi finiscono all'orizzonte (lunghezza = 600 per tutti → varianza 0), oppure una
    # variante che non cambia nulla (array identici). Trattiamoli esplicitamente:
    # nessuna variabilità entro i gruppi ⇒ nessuna evidenza da un test sulle medie.
    va = float(np.var(a)) if len(a) else 0.0
    vb = float(np.var(b)) if len(b) else 0.0
    if len(a) < 2 or len(b) < 2:
        # Dati insufficienti per stimare la dispersione: nessuna evidenza, non certezza.
        p = 1.0
    elif va == 0.0 and vb == 0.0:
        # Entrambe le serie costanti: o coincidono (nessuna differenza osservabile) o
        # sono separate in modo deterministico (limite di Welch: p → 0).
        p = 1.0 if np.mean(a) == np.mean(b) else 0.0
    else:
        # Se UNA sola serie è costante il test di Welch resta ben definito, ma scipy
        # emette comunque un RuntimeWarning di precisione sul calcolo del momento di un
        # array costante: è spurio e va silenziato, non propagato nel log.
        ...
        if not np.isfinite(p):
            p = 1.0
    if len(a) == 0 or len(b) == 0:
        # Nessun dato: restituisci un confronto vuoto invece di propagare NaN e warning.
        return Comparison(
            name_a=name_a, name_b=name_b,
            diff=Interval(point=float("nan"), lo=float("nan"), hi=float("nan"), conf=conf),
            p_value=1.0, test_name="Welch t-test (dati assenti)",
            effect_size=float("nan"), effect_label="Cohen's d",
            prob_improvement=float("nan"))
    rng = np.random.default_rng(seed)
    boots = np.array([
