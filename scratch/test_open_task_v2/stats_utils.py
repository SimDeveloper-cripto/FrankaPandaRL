...
"""
La comunità scientifica raccomanda di riportare **stime a intervallo** e metriche robuste:
  ...
  [A] Agarwal, Schwarzer, Castro, Courville, Bellemare (2021).
      ...
      → Interquartile Mean (IQM), stratified bootstrap CI, performance profiles, probability of improvement, optimality gap.
...
def wilson_ci(successes: int, n: int, conf: float = 0.95) -> Interval:
    ...
    decine di episodi.
    Per 50/50 successi Wald darebbe [1.0, 1.0] (intervallo nullo, falsamente certo);
    Wilson restituisce un limite inferiore < 1 onesto. Rif. [G].
    """
    if n <= 0:
...
def required_n_for_proportion(p_expected: float, half_width: float, conf: float = 0.95) -> int:
    ...
    semi-ampiezza `half_width`. Rif. [B].
...
def iqm(samples: Sequence[float]) -> float:
    ...
    il 25% più alto). Più robusta della media agli outlier e più efficiente della mediana.
    Metrica aggregata raccomandata in [A].
    """
...
def probability_of_improvement(a: Sequence[float], b: Sequence[float]) -> float:
    ...
    Metrica di confronto raccomandata in [A] — interpretabile senza assunzioni distribuzionali.
    """
    a = np.asarray(a, float)
    b = np.asarray(b, float)
    if len(a) == 0 or len(b) == 0:
...
def compare_continuous(
    ...
    if len(a) == 0 or len(b) == 0:
        return Comparison(
            ...
            effect_size = float("nan"), effect_label = "Cohen's d", prob_improvement = float("nan"))
...
def holm_bonferroni(p_values: Sequence[float]) -> list[float]:
    """
    Correzione di Holm-Bonferroni per confronti multipli. Rif. [C].
    """
