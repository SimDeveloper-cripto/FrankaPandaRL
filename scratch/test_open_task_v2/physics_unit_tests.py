...
Non coinvolgono la policy: verificano che il TASK sia fisicamente ben posto.
Se uno di questi fallisce, nessun risultato di RL sopra è interpretabile (Henderson et al. 2018:
gran parte della varianza attribuita agli algoritmi è in realtà dell'ambiente).

  T1 — MOLLA DEL LATCH. La leva, rilasciata a fondo corsa, torna a neutro in tempo finito.
       È la precondizione dell'uscita PULITA del RETREAT: reward_v2 termina
...
from _common import (find_repo_root, make_raw_env, results_dir, setup_matplotlib, safe_hist, json_default, resolve_run_dir)
...
def _make_rs_door():
    return suite.make("Door", robots = "Panda",
                    has_renderer           = False,
...
def _ranges_ok(r):
    return (0.013 <= r["radius"].min() and r["radius"].max() <= 0.029
            ...
            and all(r[k].std() > 1e-5 for k in ["radius", "friction", "latch_ratio", "damp_ratio", "mass_ratio"]))
