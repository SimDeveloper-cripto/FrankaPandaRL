...
su rollout seedati. Le grandezze sono scelte per discriminare le cause documentate nei commenti del sorgente, non a caso:
  ...
  T5 — STATO ALLA TRANSIZIONE HOLD_OPEN→RETREAT: open_error (la porta è davvero al goal
       quando si inizia il ritiro?) e latch_qpos (la leva è a fondo corsa, ~1.57 rad: è il carico che il riporto §1.46 deve scaricare).
...
from _common import (make_raw_env, load_obs_rms, load_model, rollout_episode,
                     results_dir, setup_matplotlib, safe_hist, json_default, resolve_run_dir, CURRICULUM, STUCK_MOVE_THRESH)
...
def run(n_episodes, deterministic, curriculum, run_dir, base_seed=30_000, tag=None):
    ...
    for i in range(n_episodes):
        rec        = rollout_episode(env, model, obs_rms, deterministic=deterministic, seed=base_seed + i, collect_trace=True)
        ...
        dev_per_ep.append((sum(1 for _, e, _ in rec.regress_events if e > 0), sum(1 for _, e, _ in rec.regress_events if e < 0)))
    ...
    out = dict(mode=("det" if deterministic else "sto"), curriculum=curriculum, n_episodes=n_episodes,
