...
Metodo: STRATIFICAZIONE. Si raccolgono molti episodi con la randomization naturale
dell'ambiente, si registra il parametro REALIZZATO e l'esito (true success), si
raggruppa in bin per quantili e si calcola il tasso con intervallo di Wilson per bin.
Rispetto a forzare un parametro per volta, questo evita di uscire dalla distribuzione
su cui la policy è stata addestrata (che falserebbe l'esito) e costa un solo giro.

Assi (7): i cinque fisici comuni alla chiusura, la distanza della porta e — SPECIFICO
DELL'APERTURA — il `goal_angle` campionato a ogni episodio: è la difficoltà del task
stesso (quanto in là bisogna aprire), e senza di esso la "generalizzazione al goal"
resterebbe non misurata.

Heatmap 2D (3): frizione×raggio (presa), rigidità latch×massa porta (dinamica del
ritiro), goal×rigidità latch (quanto in là si apre contro quanto forte richiude).
...
from _common import (make_raw_env, load_obs_rms, load_model, rollout_episode,
                     results_dir, setup_matplotlib, json_default, resolve_run_dir,
                     CURRICULUM)
...
def collect(n_episodes, curriculum, run_dir, deterministic, base_seed=40_000):
    ...
    for i in range(n_episodes):
        recs.append(rollout_episode(env, model, obs_rms, deterministic=deterministic,
                                    seed=base_seed + i))
        if (i + 1) % 25 == 0:
