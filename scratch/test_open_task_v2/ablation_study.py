...
"""
ablation_study — Confronto controllato baseline vs ablazioni degli override
deterministici del RETREAT (task APERTURA v2).

Disegno APPAIATO: ogni variante e il baseline sono valutati sugli STESSI seed, quindi
sulle stesse condizioni iniziali (goal, posa, fisica). A parità di episodi questo riduce
la varianza del confronto rispetto a due campioni indipendenti (Colas et al. 2018;
...
def eval_variant(name, overrides, n_episodes, curriculum, run_dir, deterministic, base_seed):
    ...
    recs = [rollout_episode(env, model, obs_rms, deterministic=deterministic,
                            seed=base_seed + i) for i in range(n_episodes)]
    try:
        env.close()
    except Exception:
        pass
...
def run(n_episodes, curriculum, run_dir, deterministic=True, base_seed=50_000,
    ...
    for name in names_cmp:
        ...
        comparisons[name] = dict(
            ...
            clean_success=dict(variant=v_clean, baseline=b_clean, n=n_episodes,
                               diff=cc.diff.as_dict(), fisher_p=cc.p_value),
            length=dict(diff=cl.diff.as_dict(), welch_p=cl.p_value,
                        cohens_d=cl.effect_size, prob_improvement=cl.prob_improvement),
            retreat_moved=dict(diff=cm.diff.as_dict(), welch_p=cm.p_value,
                               cohens_d=cm.effect_size))
