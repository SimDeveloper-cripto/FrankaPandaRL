...
def run_test():
    ...
    successes                           = 0
    lengths                             = []
    latch_neutral_at_transition_count   = 0
    ...
    for ep in range(10):
        ...
        transition_latch_qpos = None
        was_ready_retreat     = False
        ...
        while not done:
            ...
            ready_retreat = infos[0].get("ready_retreat", False)
            if ready_retreat and not was_ready_retreat:
                transition_latch_qpos = infos[0].get("latch_qpos")
                was_ready_retreat     = True

        is_success = infos[0].get("is_success", False)
        lengths.append(steps)
        print(f"Episode {ep+1} - Steps: {steps} - Success: {is_success} - Latch qpos at transition: {transition_latch_qpos}")
        if is_success:
            successes += 1
        if transition_latch_qpos is not None and abs(transition_latch_qpos) < 0.15:
            latch_neutral_at_transition_count += 1
            
    print(f"Total Success Rate: {successes / 10 * 100:.1f}%")
    print(f"Average Steps: {np.mean(lengths):.1f}")
    print(f"Latch Neutral at Transition Rate: {latch_neutral_at_transition_count / 10 * 100:.1f}%")
