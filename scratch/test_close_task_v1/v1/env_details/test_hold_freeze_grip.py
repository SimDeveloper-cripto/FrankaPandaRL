...
class HoldFreezeGripDoorEnv(GeneralizedDoorEnv):
    def step(self, action: np.ndarray):
        ...
        # Grab gripper action for the transition condition check
        ...
        if door_angle <= self._success_angle and not self._success_latched:
            # Require the gripper to be firmly closed (action > 0.80) to ensure a deep, stable grip at transition
            if gripper_action > 0.80:
...
def run_test():
    ...
    raw_env = HoldFreezeGripDoorEnv(cfg)
