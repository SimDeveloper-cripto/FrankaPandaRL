...
class DoorState:
    ...
    def _lavoro(self) -> float:
        ...
        §4 — LA LEVA HA UN VERSO. Con `abs(self.leva)` alzare e abbassare la
        maniglia valgono lo stesso: il segno non e' specificato da nessuna parte
        e lo decide l'inizializzazione della rete. Misurato sul modello MuJoCo:
        il giunto `Door_latch_joint` porta la maniglia a z = 1.000 con leva +1.5
        e a z = 1.150 con leva −1.5, contro z = 1.075 a riposo. Positivo e'
        ABBASSARE, come una maniglia vera.

        Qui la leva conta solo nel verso dichiarato. Non e' un termine nuovo: e'
        lo stesso `progress`, con il verso specificato invece che lasciato al
        caso. Vale per i due compiti, perche' e' geometria della porta.
        """
        ...
        return (k * min(leva_utile, self.leva_rilascio)
    ...
    def reset(self, theta_zero: float, theta_star: float, tol: float,
              leva_rilascio: float = 0.0, quota_leva: float = 0.30,
              verso_leva: float = +1.0) -> None:
